...
This document outlines the advanced features from Claude Code that have been successfully implemented in Bo CLI.
...
- Manual snapshots stored in `~/.bo/sessions/<id>.json`
...
**Features:**
- Drop JSON definitions into `~/.bo/commands/` to register `/standup`, `/deploy`, etc.
...
The implemented features follow Bo CLI's existing architecture:
- ✅ Compatible with profile system (general, bo-code)
