...
export class InteractiveShell {
  ...
  private async processSlashCommand(input: string): Promise<void> {
    ...
    switch (command) {
      ...
      case '/accept':
        await this.handleDeltaDecision(input, 'accept');
        break;
      case '/reject':
        await this.handleDeltaDecision(input, 'reject');
        break;
      case '/artifact': {
        const [, id] = input.trim().split(/\s+/, 2);
        await this.showArtifact(id);
        break;
      }
      case '/apply': {
        const [, id] = input.trim().split(/\s+/, 2);
        await this.applyArtifact(id);
        break;
      }
      case '/context':
  ...
  private async handleDeltaDecision(input: string, decision: 'accept' | 'reject'): Promise<void> {
    ...
    if (!target) {
      display.showWarning(`Usage: /${decision} <target_id>`);
      return;
    }
    ...
    const missionManager = getSharedMissionManager();
    const status = decision === 'accept' ? 'succeeded' : 'failed';
    const updated = missionManager.updateNodeStatus(target, status as any);
    getSharedTimeline().record({
      action: `delta.${decision}`,
      status: decision === 'accept' ? 'succeeded' : 'failed',
      message: target,
    });
    if (updated) {
      ...
      this.uiAdapter.showFeedbackPacket(missionManager.getFeedbackPacket());
    } else {
      display.showWarning(`Could not find plan node for "${target}".`);
    }
  }

  private async showArtifact(id?: string): Promise<void> {
    if (!id?.trim()) {
      display.showWarning('Usage: /artifact <id|path>');
      return;
    }
    ...
    if (match) {
      ...
      for (const key of ['artifact', 'diff', 'details', 'message']) {
        if ((match as any)[key]) {
          lines.push(`${key}: ${(match as any)[key]}`);
        }
      }
    ...
    const events = getSharedTimeline().list().reverse();
    const event = events.find((evt) => (evt.artifacts ?? []).includes(trimmed));
    if (event) {
      display.showSystemMessage(
        [
          `Artifact ${trimmed} found in timeline event ${event.eventId} (${event.action}).`,
  ...
  private async applyArtifact(id?: string): Promise<void> {
    if (!id?.trim()) {
      display.showWarning('Usage: /apply <artifactId|path>');
      return;
    }
    const result = this.uiAdapter.applyArtifact(id.trim());
    display.showSystemMessage(result);
  }
