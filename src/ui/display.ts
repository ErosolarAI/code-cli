...
export class Display {
  ...
  updateThinking(message: string) {
    if (this.activeSpinner) {
      this.activeSpinner.update({ text: theme.info(message) });
    } else {
  ...
  private buildClaudeStyleBanner(
    ...
    const border =
      providerAccent.edge ??
      (theme.gradient?.horizon as ((value: string) => string) | undefined) ??
      (theme.gradient?.primary as ((value: string) => string) | undefined) ??
      theme.primary;
  ...
  private buildBannerLine(
    ...
    const fill =
      ...
      (theme.ui.background as ((value: string) => string) | undefined) ??
      ((value: string) => value);
    const tinted = fill(theme.ui.text(padded));
    const resolvedEdge =
      edgeColor ??
      (theme.ui.panelEdge as ((value: string) => string) | undefined) ??
      (theme.gradient.primary as ((value: string) => string) | undefined) ??
      theme.ui.border;
