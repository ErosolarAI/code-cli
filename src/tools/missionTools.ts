  ...
  return [
    {
      name: 'SetMission',
      description:
        'Sets the agent\'s high-level objective or "mission". This will begin the autonomous execution loop.',
      parameters: {
        type: 'object',
        properties: {
          mission: {
            type: 'string',
            description:
              'A clear and concise description of the overall mission.',
          },
          task_spec: {
            type: 'object',
            description:
              'Optional structured task spec contract containing goal, constraints, budget, and risk profile.',
          },
        },
        required: ['mission'],
      },
    ...
    },
    {
      name: 'SetTaskSpec',
      description:
        'Captures a structured task contract including goal, constraints, budget, risk profile, and evaluation hints.',
      parameters: {
        type: 'object',
        properties: {
          spec: {
    ...
    },
    {
      name: 'CreatePlan',
      description:
        'Creates a new plan to work towards the current mission. Supports linear steps or a DAG.',
      parameters: {
        type: 'object',
        properties: {
          steps: {
            type: 'array',
            description:
              'A list of tasks to be executed to achieve the mission.',
            items: {
              type: 'string',
            },
          },
    ...
    },
    {
      ...
      description:
        'Returns a structured feedback packet (summary, deltas, errors, timeline refs).',
      parameters: {
        type: 'object',
        properties: {},
      },
    ...
    },
    {
      ...
      description:
        'Shows the active guardrail policy, task spec snapshot, and risk profile.',
      parameters: {
        type: 'object',
        properties: {},
      },
      handler: async () => {
        ...
        const config = policy.getConfigSnapshot
          ? policy.getConfigSnapshot()
          : {};
    ...
    },
    {
      ...
      description:
        'Updates guardrail policy (allowlist, denylist, max runtime/file size) for the current session.',
      parameters: {
        type: 'object',
        properties: {
          toolAllowlist: {
            type: 'array',
            items: { type: 'string' },
            description: 'Only allow these tools (empty to clear allowlist).',
          },
          toolDenylist: {
            type: 'array',
            items: { type: 'string' },
          ...
          maxRuntimeMs: {
            type: 'number',
            description:
              'Maximum allowed timeout in milliseconds for tool calls.',
          },
          maxFileSizeBytes: {
            type: 'number',
            description:
              'Maximum allowed content size for write operations in bytes.',
          },
        },
      },
      handler: async (args) => {
        ...
        if (policy.setConfig) {
          policy.setConfig({
            toolAllowlist: Array.isArray(args['toolAllowlist'])
              ? (args['toolAllowlist'] as string[])
              : undefined,
            toolDenylist: Array.isArray(args['toolDenylist'])
              ? (args['toolDenylist'] as string[])
              : undefined,
            maxRuntimeMs:
              typeof args['maxRuntimeMs'] === 'number'
                ? args['maxRuntimeMs']
                : undefined,
            maxFileSizeBytes:
              typeof args['maxFileSizeBytes'] === 'number'
          ...
          getSharedTimeline().record({
            ...
            metadata: {
              config: policy.getConfigSnapshot
                ? policy.getConfigSnapshot()
                : {},
            },
          });
