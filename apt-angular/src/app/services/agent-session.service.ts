...
export class AgentSessionService {
  ...
  private readonly passphraseStorageKey = 'bo-session-passphrase';
  private readonly legacyPassphraseStorageKey = 'apt-session-passphrase';
  private readonly passphraseState = signal<string>(this.readStoredPassphrase());
  private readonly firebaseTokenStorageKey = 'bo-session-firebase-token';
  private readonly legacyFirebaseTokenStorageKey = 'apt-session-firebase-token';
  ...
  setPassphrase(passphrase: string): void {
    ...
    if (this.isBrowser) {
      ...
      localStorage.removeItem(this.legacyPassphraseStorageKey);
    }
    this.triggerAccessSync(true);
  }
  ...
  setFirebaseToken(token: string | null): void {
    ...
    if (token) {
      ...
      localStorage.removeItem(this.legacyFirebaseTokenStorageKey);
    } else {
  ...
  private readStoredPassphrase(): string {
    ...
    return (
      localStorage.getItem(this.passphraseStorageKey) ??
      localStorage.getItem(this.legacyPassphraseStorageKey) ??
      ''
    );
  }
  ...
  private readStoredFirebaseToken(): string | null {
    ...
    return (
      localStorage.getItem(this.firebaseTokenStorageKey) ??
      localStorage.getItem(this.legacyFirebaseTokenStorageKey) ??
      null
    );
  }
