...
const MIRROR_SUBDIR = ['.bo-ui', 'mirror'];
...
const deriveMirrorSessionId = (filePath: string, fallback?: string): string => {
  ...
  return base.replace(/\.jsonl$/i, '') || 'bo-mirror';
};

const mirrorFileFromEnv = (): string | undefined =>
  process.env['BO_UI_MIRROR_FILE'] ??
...
export const loadRuntimeConfig = (): RuntimeConfig => {
  ...
  if (source === 'mirror-file') {
    ...
    if (!file) {
      throw new Error(
        'Set BO_UI_MIRROR_FILE (or MIRROR_FILE) or run bo-ui so we can find a mirror under <workspace>/.bo-ui/mirror/*.jsonl (legacy APT_UI_* is still honored).',
      );
    }
