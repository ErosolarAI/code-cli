# Angular SSR Bo mirror

> Commands in this document assume you are inside the `apt-angular/` folder at the repo root.

This app mirrors the dual-profile feed that the Bo CLI (general + Bo Code) prints inside the terminal. Angular now runs in SSR mode so the Express host can keep an in-process session open with whatever agent backend you point it at (local CLI, mock data, or a remote Cloud Run instance).
...
From the repo root you can also run `./scripts/full-stack.sh` to automate the installation + build + SSR launch process. The script wires `apt-ui` into the backend using `AGENT_SOURCE=local-cli`, so the dashboard is immediately connected to a live Bo session without any manual environment juggling.

The `ui:cli` script behaves like running `apt-ui` globally—you can forward any Bo CLI arguments after the `--`.
...
1. **Terminal mirror** – the hero grid mirrors the Bo CLI (general + Bo Code) (mock data by default or the real CLI when you flip `AGENT_SOURCE=local-cli`).
...
2. Bootstrap the Firestore `projects/<id>` document and default connector catalog so security rules have something to validate:
   ...
   ```
   The script lives in `functions/scripts/bootstrap-project.mjs` and seeds a mock gateway, the local Bo CLI bridge, and a `firebase-cloud-agent` connector that knows how to deploy Hosting + Functions.
...
- Open the “Session settings” drawer (top of the hero) to manage session id, passphrase, and Firebase tokens. If you expose a Firebase config via `window.BO_FIREBASE_CONFIG (legacy APT_FIREBASE_CONFIG still works)` the drawer enables a Google sign-in button; the resulting ID token is forwarded to the backend and stored securely via cookies.
...
export BO_UI_BACKEND_URL=http://localhost:4000
npm start
```
...
- `src/backend/gateways/*` exposes pluggable connectors:
  ...
  - `LocalCliGateway` spawns your local `bo --profile <name> --json …` (or any CLI that emits line-delimited JSON) and forwards stdout/stderr into the session.
  - `RemoteAgentGateway` polls a remote HTTP endpoint (for Cloud Run or another SSR host) so you can daisy-chain Bo mirrors.
...
LOCAL_CLI_COMMAND="bo --profile bo-code --json" \
LOCAL_CLI_CWD=$PWD \
npm run serve:ssr
```
...
```bash
AGENT_SOURCE=local-cli \
LOCAL_CLI_COMMAND="npm run ui:cli -- bo --profile general --json" \
...
The gateway writes whatever you type into the browser input back to the CLI process via stdin, so you can keep any Bo CLI profile running headless and steer it from the mirror.
...
The remote instance just needs to expose the same `/api/session` + `/api/session/stream` contract, so you can chain SSR pods or attach to `bo share --live` output.

### Mirror an existing Bo CLI session

Launch Bo CLI through `apt-ui` (or any process that writes the JSONL mirror) and point the Angular server at the file it prints in the banner:

```
AGENT_SOURCE=mirror-file \
BO_UI_MIRROR_FILE=/Users/bo/GitHub/workspace/.bo-ui/mirror/apt-2025-11-16T17-20-58-852Z.jsonl \
npm run dev:ssr
```

The backend tails the log, emits every stdout/stderr line as chat messages, and surfaces process lifecycle events in the ops panel. Set `BO_UI_SESSION_ID` (or `SESSION_ID`) if you want the dashboard to label the session differently from the filename. If you omit `BO_UI_MIRROR_FILE` the server now looks for the newest `.jsonl` inside `<workspace>/.bo-ui/mirror/`, where the workspace is derived from `BO_UI_WORKSPACE`, `WORKSPACE`, or the current working directory. That means you can simply run `apt-ui` in the same repo and start the Angular server without any extra configuration—the freshest mirror file will be picked up automatically.
...
- Signals keep the DOM in sync as SSE events arrive, so the UI behaves like the tmux feed you see in Bo CLI.
