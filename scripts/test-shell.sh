...
if [ -z "$OPENAI_API_KEY" ]; then
    echo "⚠ OPENAI_API_KEY not set (Bo Code profile will not work)"
fi
echo ""

# Test help command
echo "✓ Testing help command..."
if echo "help" | timeout 5 node dist/bin/bo.js 2>&1 | grep -q "Available Tools"; then
...
HELP_OUTPUT=$(echo "help" | timeout 5 node dist/bin/bo.js 2>&1)
...
if BO_CODE_MODEL="gpt-4" node dist/bin/bo.js 2>&1 | grep -q "gpt-5.1-codex"; then
