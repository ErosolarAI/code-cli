...
console.log(
  `${colors.cyan}🧪 Bo CLI Comprehensive Health Check${colors.reset}\n`,
);
...
if (packageJson) {
  ...
  // Check Bo-specific configuration (falling back to legacy keys)
  const rulebookSchema =
    packageJson.bo?.rulebookSchema ?? packageJson.apt?.rulebookSchema ?? null;
  if (rulebookSchema) {
    logSuccess('Bo rulebook schema configured');
  } else {
    logWarning('Bo rulebook schema not configured in package.json');
    hasWarnings = true;
  }
}
...
if (checkFileExists('dist', 'Build directory')) {
  // Check main binary
  if (checkFileExists('dist/bin/bo.js', 'Main binary')) {
    // Check bin file permissions
    const binCheck = spawnSync('ls', ['-la', 'dist/bin/bo.js'], {
      encoding: 'utf8',
    });
...
const agentFiles = [
  { path: 'agents/bo-code.rules.json', name: 'Bo Code rulebook' },
...
agentFiles.forEach(({ path, name }) => {
  ...
  if (content) {
    ...
    if (missingFields.length === 0) {
      ...
      logError(
        `${name} - Missing required fields: ${missingFields.join(', ')}`,
      );
      hasErrors = true;
    }
  }
...
checkFileExists(
  'src/contracts/schemas/agent-rules.schema.json',
  'Agent rules schema',
);
...
if (hasErrors) {
  ...
  console.log(
    `${colors.yellow}💡 Run 'npm run build' to rebuild the project${colors.reset}`,
  );
  console.log(
    `${colors.yellow}💡 Run 'npm test' to verify functionality${colors.reset}`,
  );
  process.exit(1);
} else if (hasWarnings) {
  console.log(
    `${colors.yellow}⚠️ Health check completed with warnings${colors.reset}`,
  );
  console.log(
    `${colors.yellow}💡 Review warnings above and address as needed${colors.reset}`,
  );
  process.exit(0);
} else {
  console.log(
    `${colors.green}🎉 Health check passed successfully!${colors.reset}`,
  );
  console.log(`${colors.green}✅ The CLI is ready for use${colors.reset}`);
  console.log(`\n${colors.cyan}💡 Next steps:${colors.reset}`);
  console.log("   Run 'bo' to start the CLI");
